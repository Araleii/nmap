...
NMAP_PLATFORM=x86_64-apple-darwin15.0.0
...
LIBS =  -lnsock -lnbase libpcre/libpcre.a -lpcap $(OPENSSL_LIBS) libnetutil/libnetutil.a $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a $(top_srcdir)/liblua/liblua.a $(top_srcdir)/liblinear/liblinear.a -lm 
OPENSSL_LIBS = 
# LIBS =  -lefence -lm 
# LIBS =  -lrmalloc -lm 
...
$(TARGET): build-netutil build-liblinear  build-pcre build-nsock build-nbase build-dnet build-lua \
	$(OBJS) main.o
	@echo Compiling nmap
	rm -f $@
...
clean: clean-lua clean-liblinear  clean-pcre clean-dnet \
